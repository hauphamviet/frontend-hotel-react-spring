...
export async function deleteRoom(roomId) {
    try {
        const resutl = await api.delete(`api/rooms/delete/room/${roomId}`)
        return resutl.data
    } catch (error) {
...
export async function bookRoom(roomId, booking) {
    try {
        //    @PostMapping("/api/bookings/room/{roomId}/booking")
...
// This is function gets all available rooms from the database with a given date and a room type
export async function getAvailableRooms(checkInDate, checkOutDate, roomType) {
	const result = await api.get(
		`api/rooms/available-rooms?checkInDate=${checkInDate}
